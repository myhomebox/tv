...
async function category(tid, pg, filter, extend) {
    if (pg <= 0) pg = 1;
    let link = url + '/vodshow/';
    link += jinja2('{{tid}}-{{ext.area}}-{{ext.by}}-{{ext.class}}-{{ext.lang}}-{{ext.letter}}---{{pg}}---{{ext.year}}.html', { ext: extend, tid: extend.tid ? extend.tid : tid, pg: pg });
    const html = await request(link);
    const $ = load(html);
    const items = $('div.myui-panel_bd > ul.myui-vodlist > li');
    let videos = _.map(items, (item) => {
