function getAddress(field) {
    ...
    if (fabu === null) {
        ...
        res = post.call({ url: 'http://103.45.68.47:6800/gy/gy_urkP/p3p端口.php', headers: JSON.stringify(headers), body: 'captcha=123' });

        result = />p3p端口：<a href="(.*?)"/.exec(res);

        if (!result) {
            return JSON.stringify({ error: '取得接口出錯！' });
        }
    ...
    if (!result) {
        return JSON.stringify({ error: '取得端口出錯！' });
    }
    ...
    return JSON.stringify({ error: '列表不正確！' });
}
