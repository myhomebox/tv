...
#    cookies = ''
    ...
    def categoryContent(self, tid, pg, filter, extend):
        result = {}
        url = 'https://czzy.pro/{0}/page/{1}'.format(tid,pg)
        hostText = self.getCookie(url)
        root = self.html(self.cleanText(hostText))
    ...
    def detailContent(self, array):
        tid = array[0]
        url = 'https://czzy.pro/movie/{0}.html'.format(tid)
        hostText = self.getCookie(url)
        root = self.html(self.cleanText(hostText))
    ...
    def searchContent(self, key, quick):
        url = 'https://czzy.pro/xssearch?q={0}'.format(key)
        hostText = self.getCookie(url)
        root = self.html(self.cleanText(hostText))
    ...
    def playerContent(self, flag, id, vipFlags):
        result = {}
        url = 'https://czzy.pro/v_play/{0}.html'.format(id)
