...
class Spider(Spider):
    ...
    def homeVideoContent(self):
        a = []
        try:
            res = requests.get(url=self.home_url + '/api/crumb/list?fcate_pid=1&category_id=5&area=0&year=0&limit=24&channel=wandoujia&page=1&sort=hot&type=0', headers=self.headers)
    ...
    def categoryContent(self, cid, page, filter, ext):
        a = []
        try:
            res = requests.get(url=self.home_url + f'/api/crumb/list?fcate_pid=1&category_id={cid}&area=0&year=0&limit=24&channel=wandoujia&page={page}&sort=hot&type=0',
