...
install_Fourgtv() {
    ...
    IMAGE_SOURCE="lru2025/fourgtv"
    PROXY_IMAGE_SOURCE="$REVERSE_PROXY/lru2025/fourgtv"
    ...
    if docker ps -a --format '{{.Names}}' | grep -q "^fourgtv$"; then
        ...
        docker images --format '{{.Repository}}:{{.Tag}}' | grep 'lru2025/fourgtv' | xargs -r docker rmi > /dev/null 2>&1
    fi
